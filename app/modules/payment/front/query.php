...
if($lb=="alipay"):
		...
		$row=Data::QueryData("cms_routine","","","","1","0")["querydata"][0];
		...
		if(!empty($resultCode)&&$resultCode == 10000):
				...
				elseif($trade_status=='TRADE_SUCCESS'):
						Data::UpdateData("cms_pay_log",array("state"=>1,"paynum"=>$trade_no,"paytime"=>date('Y-m-d H:i:s',time())),"postnum='$postnum' and state=0");
...
elseif($lb=="wechat"):
    ...
    if(isset($_REQUEST["transaction_id"]) && $_REQUEST["transaction_id"] != ""):
        $transaction_id = Inc::SqlCheck($_REQUEST["transaction_id"]);
    ...
    if(isset($_REQUEST["out_trade_no"]) && $_REQUEST["out_trade_no"] != ""):
        $out_trade_no = Inc::SqlCheck($_REQUEST["out_trade_no"]);
        $input = new WxPayOrderQuery();
