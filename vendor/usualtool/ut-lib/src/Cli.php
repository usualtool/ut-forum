...
class Cli{
    ...
    public static function Run($array){
        ...
        if(method_exists(Cli::class,$cli)){
            Cli::$cli($array);
        }else{
    ...
    public static function Module($array){
        if(count($array)>2){
            $module=$array[2];
            $m1=Inc::MakeDir(APP_ROOT."/modules/".$module,0777);
            $m2=Inc::MakeDir(APP_ROOT."/modules/".$module."/admin",0777);
            $m3=Inc::MakeDir(APP_ROOT."/modules/".$module."/cache",0777);
            $m4=Inc::MakeDir(APP_ROOT."/modules/".$module."/front",0777);
            $m5=Inc::MakeDir(APP_ROOT."/modules/".$module."/model",0777);
            $m6=Inc::MakeDir(APP_ROOT."/modules/".$module."/skin",0777);
            $m7=Inc::MakeDir(APP_ROOT."/modules/".$module."/skin/admin",0777);
            $m8=Inc::MakeDir(APP_ROOT."/modules/".$module."/skin/front",0777);
    ...
    public static function Plugin($array){
        if(count($array)>2){
            $plugin=$array[2];
            $p=Inc::MakeDir(APP_ROOT."/plugins/".$plugin,0777);
            if($p){
                $c="<?xml version='1.0' encoding='UTF-8'?>\r\n";
    ...
    public static function Key(){
        ...
        $key=$config["UTCODE"];
        echo Inc::Auth($config["UTCODE"],$config["UTFURL"],"key")."\r\n";
    }
    /**
     * 安装命令
     * @param array $array
        ...
        if(count($array)>2){
            ...
            if($type=="module"){
                ...
                if($number!="-2"){
                    $down="";
                    if($number=="-1"){
                        $down=Inc::Auth($config["UTCODE"],$config["UTFURL"],"module-".$name);
                    }elseif($number=="-3"){ 
                        $down=Inc::Auth($config["UTCODE"],$config["UTFURL"],"moduleorder-".$name);
                    }
                ...
                if(is_dir(APP_ROOT."/modules/".$name."/assets")){
                    ...
                    Inc::MoveDir(APP_ROOT."/modules/".$name."/assets",$assets_dir);
                    Inc::DelDir(APP_ROOT."/modules/".$name."/assets");
                }
                ...
                if(Data::ModTable("cms_admin_role") && Data::ModTable("cms_module")){
                ...
                if($number!="-2"){
                    $down="";
                    if($number=="-1"){
                        $down=Inc::Auth($config["UTCODE"],$config["UTFURL"],"plugin-".$name);
                    }elseif($number=="-3"){  
                        $down=Inc::Auth($config["UTCODE"],$config["UTFURL"],"pluginorder-".$name);
                    }
                    $downurl=Inc::StrSubstr("<downurl>","</downurl>",$down);
                    $filename=basename($downurl);
                ...
                if(is_dir(APP_ROOT."/plugins/".$name."/assets")){
                    ...
                    Inc::MoveDir(APP_ROOT."/plugins/".$name."/assets",$assets_dir);
                    Inc::DelDir(APP_ROOT."/plugins/".$name."/assets");
                }
                ...
                if($number!="-2"){
                    $down="";
                    if($number=="-1"){
                        $down=Inc::Auth($config["UTCODE"],$config["UTFURL"],"temp_".$name);
                    }elseif($number=="-3"){  
                        $down=Inc::Auth($config["UTCODE"],$config["UTFURL"],"temporder_".$name);
                    }
                    $downurl=Inc::StrSubstr("<downurl>","</downurl>",$down);
                    $filename=basename($downurl);
                ...
                if(is_dir(APP_ROOT."/template/".$name."/move")){
                    echo"正在移动模型文件...\r\n";
                    Inc::MoveDir(APP_ROOT."/template/".$name."/move",UTF_ROOT);
                }
                if(is_dir(APP_ROOT."/template/".$name."/assets")){
                    ...
                    Inc::MoveDir(APP_ROOT."/template/".$name."/assets",$assets_dir);
                    Inc::DelDir(APP_ROOT."/template/".$name."/assets");
                }
                ...
                $module=Inc::StrSubstr("<module>","</module>",$template);
                if(Data::ModTable("cms_template")){
                    if(Data::QueryData("cms_template","","tid='$name'","","1")["querynum"]>0){
                        Data::UpdateData("cms_template",array("tid"=>$name,"lang"=>$lang,"title"=>$title),"tid='$name'");
                    }else{
                        Data::InsertData("cms_template",array("tid"=>$name,"lang"=>$lang,"title"=>$title));
                    }
                }
                if(!empty($module)){
                    $mod=explode(",",$module);
                    for($i=0;$i<count($mod);$i++){
                        cli::Install(array("usualtool","install","module",$mod[$i],"-1"));
                    }
                }
                if($installsql=='0'){
                    echo"成功安装".$name."模板\r\n";
    ...
    public static function Composer($array){
        ...
        foreach($alldirs as $utpath){
            ...
            $new_version = Inc::GetVer($new_configfile);
            if(!$new_version){
            ...
            if(file_exists($old_configfile)){
                $old_version = Inc::GetVer($old_configfile);
            }
            ...
            if(is_dir($target_moduledir)){
                Inc::DelDir($target_moduledir);
            }
            Inc::CopyDir($source_moduledir,$target_moduledir);
            echo "Composer自动化完成\r\n";
        }
    }
    /**
