...
class Inc{
    ...
    public static function GetMod($type=0){
        ...
        if(empty($type) || $type==0){
            ...
            $modules=self::Auth($config["UTCODE"],$config["UTFURL"],"module");
            ...
            $modules=self::Auth($config["UTCODE"],$config["UTFURL"],"moduleorder");
            preg_match_all("/\<module\>(.*?)\<\/module\>/s",$modules,$moduleblocks);
            foreach($moduleblocks[1] as $module){
    ...
    public static function GetPlugin($type=0){
        ...
        if(empty($type) || $type==0){
            ...
            $hooks=self::Auth($config["UTCODE"],$config["UTFURL"],"plugin");
            ...
            $hooks=self::Auth($config["UTCODE"],$config["UTFURL"],"pluginorder");
            preg_match_all("/\<hook\>(.*?)\<\/hook\>/s",$hooks,$hookblocks);
            foreach($hookblocks[1] as $hook){
    ...
    public static function GetTemplate($type=0){
        ...
        if(empty($type) || $type==0){
              ...
              if($v!="index.html"){
                ...
                if(is_file($file."/usualtool.config")){
                    ...
                    if($temptype==2){
                        ...
                        $auther=self::StrSubstr("<auther>","</auther>",$temps);
                        $description=self::StrSubstr("<description>","</description>",$temps);
            ...
            $temps=self::Auth($config["UTCODE"],$config["UTFURL"],"temp");
            ...
            $temps=self::Auth($config["UTCODE"],$config["UTFURL"],"temporder");
            preg_match_all("/\<temp\>(.*?)\<\/temp\>/s",$temps,$tempblocks);
            foreach($tempblocks[1] as $temp){
    ...
    public static function HasComposer($modpath){
        ...
        if(!empty($missing)){
            ...
            self::GoUrl("-1",$missingstr." not installed.");
        }
    }
    /**
     * 获取配置文件中的版本号
    ...
    public static function Auth($authcode,$authurl,$apitype){
        $fromurl=self::CurPageUrl();
        $url="".$authurl."?AuthCode=".$authcode."&FromUrl=".$fromurl."&Type=".$apitype."";
        $content=self::HttpGet($url);
        return str_replace("#","$",str_replace("&","=",self::StrSubstr("<php>","</php>",$content)));
    }
    /**
     * 生成随机字符串
    ...
    public static function TempEndPath(){
        $thispage=self::CurPageUrl();
        $develop=self::GetConfig()["DEVELOP"];
    ...
    public static function Download($file){
        ...
        if(!file_exists($file)){
            ...
            Header("Content-Disposition: attachment; filename=".self::GetRandomString(8).".".substr(strrchr($filename,'.'),1));
    ...
    public static function Robot(){
        $config=self::GetConfig();
        $robot=self::HttpGet($config["UTFURL"]."?AuthCode=".$config["UTCODE"]."&FromUrl=".self::CurPageUrl()."&Type=robot");
        echo self::StrSubstr("<robot-code>","</robot-code>",$robot);
    }
}