...
class Temp{
    ...
    function TempReplace($content){
        ...
        $content=preg_replace_callback(
            ...
            function($matches){
                if(Inc::Contain("inc",$matches[1])){
                    return $this->StripTags("<?php echo usualtool\Lib\Inc::$matches[2];?>");
                }elseif(Inc::Contain("db",$matches[1])){
                    return $this->StripTags("<?php echo usualtool\Lib\Data::$matches[2];?>");
                }elseif(Inc::Contain("url",$matches[1])){
                    return $this->StripTags("<?php echo usualtool\Lib\Route::$matches[2];?>");
                }
            },
            $content
        );
