...
class Lang{
    ...
    public static function GetLang($path=OPEN_ROOT."/lang/"){
        ...
        while(($file = readdir($current_dir)) !== false){
            ...
            }elseif(Inc::Contain("json", $file)){
                $langs[] = Inc::StrSubstr("lg-", ".json", basename($file));
            }
        }
    ...
    public static function ModLangData($word,$module=''){
        ...
        if(!empty($module)){
            ...
            $langdata=json_decode(file_get_contents($_modpath_."/lang/lg-".self::GetActiveLang().".json"),true);
        }
        if(array_key_exists($word,$langdata["l"])){
            $langword=$langdata["l"]["".$word.""];
            return $langword;
    ...
    public static function Lang($path=OPEN_ROOT.'/lang/'){
        ...
        $current_dir = opendir($path);
        while(($file = readdir($current_dir)) !== false) {
            if(Inc::Contain(".json",$file)!==false){
